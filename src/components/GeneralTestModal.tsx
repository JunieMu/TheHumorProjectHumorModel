...
import { X, Loader2, Send, CheckCircle2, AlertCircle, Image as ImageIcon } from "lucide-react";

interface GeneralTestModalProps {
  isOpen: boolean;
  onClose: () => void;
}
...
export function GeneralTestModal({ isOpen, onClose }: GeneralTestModalProps) {
  ...
  const runTest = async () => {
    ...
    try {
      ...
      const generateRes = await fetch(`${API_BASE}/pipeline/generate-captions`, {
        ...
        body: JSON.stringify({ imageId, humorFlavorId: parseInt(selectedFlavorId) }),
      });
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-vintage-gray/40 backdrop-blur-sm overflow-y-auto">
      <div className="vintage-border bg-vintage-cream w-full max-w-4xl p-8 relative my-auto shadow-2xl">
        <button
          onClick={onClose}
        ...
        <p className="text-xs font-typewriter italic text-vintage-gray/60 mb-8 pb-4 border-b border-vintage-gray/10">
          Conduct a comprehensive extraction test using any registered humor flavor.
        </p>

        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          <section className="space-y-6">
            <div className="space-y-2">
              <label className="block text-xs font-bold uppercase text-vintage-gray/70 font-typewriter">
                Select Extract Flavor
              </label>
              {metaLoading ? (
                <div className="h-10 flex items-center gap-2 italic text-xs font-typewriter">
                  <Loader2 size={14} className="animate-spin" />
                  Indexing flavor records...
                </div>
              ) : (
                <select
                  value={selectedFlavorId}
                  onChange={(e) => setSelectedFlavorId(e.target.value)}
                  className="w-full bg-vintage-cream-dark border-2 border-vintage-gray/20 focus:border-vintage-blue-dark outline-none p-2 text-sm font-typewriter"
                >
                  <option value="">-- Choose a Flavor Record --</option>
                  {flavors.map((f) => (
            ...
            <div 
              ...
              onClick={() => !loading && document.getElementById('general-test-input')?.click()}
            >
              {previewUrl ? (
                ...
                <div className="text-center p-6">
                  ...
                  <p className="text-sm font-typewriter text-vintage-gray/50 italic">
                    Submit Image for Extraction
                  </p>
                </div>
              )}
              <input 
                id="general-test-input"
                type="file" 
                className="hidden" 
                accept="image/*"
            ...
            <button
              onClick={runTest}
              disabled={!file || !selectedFlavorId || loading}
            ...
            >
              ...
              <span className="font-bold uppercase tracking-wider">Execute Run</span>
            </button>
            ...
            <div className="flex-1 vintage-border bg-white p-6 min-h-[400px] overflow-y-auto space-y-4 shadow-inner">
              {captions.length > 0 ? (
                ...
                <div className="h-full flex flex-col items-center justify-center text-center opacity-20">
                  <span className="font-typewriter text-4xl mb-4 font-bold tracking-tighter italic text-vintage-gray">NO DATA</span>
                  <p className="text-xs font-typewriter italic max-w-[200px]">
                    Waiting for pipeline execution results...
                  </p>
                </div>
              )}
            </div>
